 ...
 * seeded and replayable or fresh, `Clock` the one source of real time; `Maths` and `Polygons` are
 * the arithmetic more than one module needs, `Vec2` and `Vec3` its value objects. No DOM, no
 * three.js, no Jolt: a package that reads this stays testable on its own.
 */
export {
  EventBus,
  type EventHandler,

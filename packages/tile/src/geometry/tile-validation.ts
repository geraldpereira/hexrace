...
import { insideConvex } from '@hexrace/commons';
...
export class TileValidation {
  ...
  obstacle(sweep: TileSweep, obstacle: Obstacle): TileIssue[] {
    ...
    if (outline.some((p: SPoint) => !insideConvex(p.at, corners, SIDE * 1e-6))) {
...
function describe(obstacle: Obstacle): string {

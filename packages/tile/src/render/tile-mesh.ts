...
import { type Vec2, hexCorners } from '@tile/entity/layout';
...
export class TileMeshes {
  ...
  outline(sweep: TileSweep, color = '#0c0a09'): THREE.LineLoop {
    const points = hexCorners(sweep.center).map((p: Vec2) => {
      const v = toWorld(p, this.sweeper.heightAt(sweep, p) + OUTLINE_LIFT);

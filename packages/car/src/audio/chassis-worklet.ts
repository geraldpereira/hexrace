...
/**
 * Everything around the car that is neither the engine nor a sliding tyre (POC 1): rolling noise,
 * one voice per surface, driven by how many wheels roll on it and how fast; the wind, low-passed
 * noise that opens and swells with the speed, with gusts; and the suspension, a low knock and a
 * body creak on every fast compression or rebound, harder and metallic when it bottoms out.
 */
@Injectable({ providedIn: 'root' })
export class ChassisWorklet {
  readonly name = 'chassis-processor';
  readonly source = `
class Biquad {

...
/**
 * Tyre slide noise as a worklet, one voice per surface of the palette (POC 1). Each voice mixes
 * two models by its `tone`: a squeal, white noise through a sharp resonance whose pitch wanders
 * and rises with the slide, and a crunch, a random train of short noise grains under a low-pass
 * over a bed of rumble. The main thread sends the intensities every frame and the parameters when
 * a surface changes: asphalt whistles, gravel crackles, dirt scrapes, mud is muffled.
 */
@Injectable({ providedIn: 'root' })
export class TyreWorklet {
  readonly name = 'tyre-processor';
  readonly source = `
class Biquad {

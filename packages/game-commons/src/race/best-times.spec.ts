...
describe('BestTimes', () => {
  ...
  it('knows no time for a track it has never seen', () => {
    expect(times().best('europe-loop-01')).toBeNull();
  });

  it('saves a first time, then only what beats it', () => {
    const best = times();
    expect(best.record('europe-loop-01', 90_000)).toBe(true);
    expect(best.best('europe-loop-01')).toBe(90_000);
    expect(best.record('europe-loop-01', 95_000)).toBe(false);
    expect(best.record('europe-loop-01', 90_000)).toBe(false);
    expect(best.record('europe-loop-01', 88_000)).toBe(true);
    expect(best.best('europe-loop-01')).toBe(88_000);
  ...
  it('drops a document it cannot read rather than migrating it', () => {
    ...
    expect(times().best('europe-loop-01')).toBeNull();
    localStorage.setItem(BEST_TIMES_KEY, '{"tracks":{"europe-loop-01":"soon"}}');
    expect(times().best('europe-loop-01')).toBeNull();
  });

  it('forgets everything on demand', () => {
    const best = times();
    best.record('europe-loop-01', 90_000);
    best.clear();
    expect(best.best('europe-loop-01')).toBeNull();
  });
  ...
  it('keeps nothing without a window, and every run is then a record', () => {
    ...
    expect(best.record('europe-loop-01', 90_000)).toBe(true);
    expect(best.record('europe-loop-01', 95_000)).toBe(true);
    best.clear();
    expect(best.best('europe-loop-01')).toBeNull();
  });
});

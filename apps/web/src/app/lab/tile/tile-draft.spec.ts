...
describe('TileDraft', () => {
  it('builds a valid tile by default, with every toggled obstacle and the line', () => {
    ...
    const validation = TestBed.inject(TileValidation);
    const paths = TestBed.inject(TilePaths);
    const units = TestBed.inject(Units);
    const draft = new TileDraft();
    expect(draft.errors(validation, paths)).toEqual([]);
    ...
    expect(draft.build(paths, units).line).toBe(0.5);
    ...
    expect(draft.build(paths, units).line).toBeNull();
    draft.exitProfile.roadWidth = 9;
    expect(draft.errors(validation, paths)).toEqual([
      'exit profile: road of 9 units, expected 1 to 5',

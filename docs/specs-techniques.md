...
**Le grain est une fonction du lieu, et il a gagné un second terme (2026-09-15).** `Grain` échantillonnait `Noise` le long de la **distance parcourue**, avec un décalage constant par roue : le champ n'était attaché à aucun endroit, repasser au même endroit ne redonnait pas la même bosse, et rien à l'écran ne pouvait montrer ce que la voiture sentait. Il l'échantillonne désormais à la **position monde du contact**, `x` et `z` divisés par la longueur d'onde. Les propriétés tiennent toujours : une voiture à l'arrêt ne tremble pas puisque la position ne bouge pas, deux roues lisent des valeurs différentes parce qu'elles sont à deux endroits (le décalage artificiel par roue a disparu), et tout reste déterministe — mieux qu'avant, puisque le champ ne dépend plus de l'histoire du tour. `CarController` ne compte donc plus la distance parcourue, et `Noise` est monté dans `commons` (`commons/math`), d'où `tile` le lit pour peindre le même champ.

À côté du grain, une surface porte une **houle** : `Swell` (`tile/entity`), une hauteur et un pas en mètres, rangée par environnement, zone et rang dans `SWELL_CATALOG` (`car/entity/surfaces/`), à côté de `SURFACE_CATALOG`. Les deux bruits s'additionnent dans la bosse virtuelle poussée dans la précharge de suspension. La houle vit dans sa propre table et non dans `SurfaceFeel` parce qu'un `SurfaceFeel` est **partagé entre zones** — `TURF` est à la fois le bas-côté 2 et le paysage 1 d'Europe — alors que la houle est justement ce qui sépare le paysage de la route. Valeurs retenues : route 0 partout, bas-côté 1,5 à 3,5 cm, paysage 10 à 14 cm pour un pas de 6 à 8 m, soit une bosse tous les douze à seize mètres. Le cadrage vient d'un modèle quart de véhicule sur la suspension du spec (course 0,05 à 0,25 m, 2,5 Hz, amortissement 0,7) : à 10 cm et 7 m de pas, la caisse encaisse environ 0,9 g de crête et la roue s'allège sans décoller sous 90 km/h ; au-delà de 15 cm, ou sous 5 m de pas, la voiture décolle en permanence et la houle catapulte au lieu de punir. Mesuré en course dans `lab/race`, hors piste à 77 km/h : bosse de +11 à −2 cm sous la roue, roue en l'air 11 % du temps, voiture toujours conduisible.
...
- 6.1 Scène et caméra
  ...
  **La rugosité du sol, écrite (2026-09-15).** Le paysage ondule à l'écran sans qu'aucun sommet ne bouge : le maillage d'une tuile sert aussi de collider, et le sol doit rester exactement le sol que la spec 3.5 décrit. Ce qui bouge est la **lumière**. `TileMeshes` coupe une tuile en un maillage **par rang de zone** — un matériau ne porte qu'une rugosité, et une tuile mêle piste, bas-côtés et paysage — et laisse dans un dernier maillage tout ce qui n'est pas une zone, la jupe, les obstacles, le damier ; quatre à sept appels de rendu par tuile, sept tuiles dans la fenêtre, ce qui ne coûte rien. Chaque maillage de zone reçoit :

  - des **UV planaires tirées de la position monde**, `x` et `z` divisés par la longueur d'onde de la houle : la carte tombe donc sur le même champ, aux mêmes coordonnées, que ce que la roue subit ;
  - une **carte de normales générée par le code**, une `DataTexture` de 256 px construite depuis le même service `Noise` — le gradient du champ replié sur huit cellules (`Noise.tiled`) pour qu'elle se répète sans couture — partagée par toutes les zones, l'échelle étant portée par les UV et l'intensité par `normalScale` ;
  - une **variation de couleur par sommet**, plus douce, qui éclaircit les crêtes et assombrit les creux : la carte donne le relief qui accroche la lumière, la couleur la tache large.

  `normalScale` et la variation de couleur suivent la hauteur de la houle, saturées à `swellReference` (15 cm) : un bas-côté à 2 cm est à peine marqué, un paysage à 13 cm l'est franchement. Rien n'est peint du **grain fin** : sous le mètre de longueur d'onde, ni le maillage ni la carte ne peuvent le montrer sans devenir sale.

  **Comment la rugosité descend sans que `tile` lise `car`.** La décision du 2026-09-14 tient : `tile` ne connaît d'une surface qu'un rang et une couleur. `tile/entity` déclare seulement le contrat, `Swell` et `SwellProbe` (`of(zone, rang)` rend une houle ou `null`) ; `TileMeshes` et `TrackMeshes` prennent la sonde en option. C'est `TrackStage`, dans `game-commons`, seul endroit qui tienne les deux tables, qui fabrique la sonde à partir de `Surfaces.swell` et la passe à `TrackMeshes` puis à `TileMeshes`. Une tuile dessinée sans sonde reste dessinée, à plat, en un seul maillage : `lab/tile` et `lab/track` fabriquent la leur pour montrer le relief, et l'éteignent d'une case à cocher.
...
| 2026-09-15 | `Environment.faceRoads` borne le rang de piste que le générateur pose sur une face, 2 au Nord et 3 ailleurs ; une plaque tire un rang autre que celui d'en dessous, la glace comprise                                                                                                     | La donnée vit avec la palette, dans `tile/entity`, et `ProfileSteps` la reçoit avec le reste du pas ; les adhérences gelées montent au passage ([F 2.2], [F 5.3])                             |
...
| 2026-09-15 | `Grain` échantillonne `Noise` à la **position monde du contact** et non le long de la distance parcourue ; `Noise` monte dans `commons` | Un champ attaché au lieu redonne la même bosse au même endroit et peut être peint ; le décalage par roue et le compteur de distance disparaissent ([F 2.3]) |
| 2026-09-15 | La **houle** est une table à part, `SWELL_CATALOG` par environnement, zone et rang, et non un champ de `SurfaceFeel` | Un `SurfaceFeel` est partagé entre zones (`TURF` est bas-côté 2 et paysage 1) ; la houle est justement ce qui sépare le paysage de la route |
| 2026-09-15 | Aucun sommet ne bouge : la houle se rend par une **carte de normales générée** sur des UV planaires monde, plus une teinte par sommet | Le maillage d'une tuile est aussi son collider ; la lumière suffit à faire voir venir ([F 8.2]) et ne coûte pas un triangle |
| 2026-09-15 | Une tuile se découpe en un **maillage par rang de zone**, le reste dans un dernier, sans toucher au collider | `normalScale` est un réglage du matériau : une tuile qui mêle trois zones ne peut pas porter trois rugosités dans un seul maillage |
| 2026-09-15 | La rugosité descend par une sonde `SwellProbe` déclarée dans `tile/entity` et remplie par `TrackStage` | `tile` ne lit toujours pas le tableau de `car` (décision du 2026-09-14) ; une tuile sans sonde reste dessinée à plat |
